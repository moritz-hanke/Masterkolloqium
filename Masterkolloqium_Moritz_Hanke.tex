...
\begin{frame}
	...
	\item für $\boldsymbol{\beta} = 1$ trifft Enet im Durchschnitt fast ($1.010$) den wahren Wert; die NBPR-Versionen liegen zwischen $1.239$ ($\text{NBPR}_{B,R}$) und $1.419$ ($\text{NBPR}_1$)  
...
\begin{frame}
	Kovariablenauswahl Enet vs. $\text{NBPR}_{B,R}$ für $\boldsymbol{\beta}$
	\begin{figure}
	\centering
...
\end{frame}
\begin{frame}
	Kovariablenauswahl Enet vs. $\text{NBPR}_{B,R}$ für $\boldsymbol{\beta} = 0$
	\begin{figure}
	\centering
